...
Aphi = 1.07
Tomsh = TomDat["Data"][:,46]
TomsH = TomDat["Data"][:,57]
...
for kk in range(0,xdim):
    for kkk in range(0,ydim):
        if strike[kk,kkk] != 0:
            ...
            abs_diff = np.abs(TomDEP - Zref)
            DepIdx=np.argmin(abs_diff)
            
            if Tomsh[DepIdx] != -9999:
                ...
                μ_ = np.tan(TomMu[DepIdx]*np.pi/180)
                Pp_= TomPp[DepIdx]
...
# sio.savemat('/Users/kmccormack/CSER Dropbox/Kevin McCormack/Utah/FaultCharacterizationProject/UpdatedHazard/Paper/MATLAB_Input_hybrid.mat',OutputForMATLAB)
