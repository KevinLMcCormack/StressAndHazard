...
MyPath='/Users/kmccormack/CSER Dropbox/Kevin McCormack/Utah/FaultCharacterizationProject/UpdatedHazard/Ade_Faults2'
...
FaultsCFF=zeros((len(LD),xdim,ydim,n_sp))
KrigXYforMAT=zeros((len(LD),xdim*ydim,2))
KrigZforMAT=zeros((len(LD),xdim*ydim,1))
...
# Generate Stresses (SoSAT)
    
    import pickle
    with open('/Users/kmccormack/CSER Dropbox/Kevin McCormack/Utah/FaultCharacterizationProject/UpdatedHazard/StressForTom/SoSAT_ss.pkl', 'rb') as f:
        depth, shmin_lower, shmin_s, shmin_upper, shmax_lower, shmax_s, shmax_upper = pickle.load(f)
    
    
    
    shmin_lower=np.array(shmin_lower,dtype='float')
    shmin_s=np.array(shmin_s,dtype='float')
    shmin_upper=np.array(shmin_upper,dtype='float')
    shmax_lower=np.array(shmax_lower,dtype='float')
    shmax_s=np.array(shmax_s,dtype='float')
    shmax_upper=np.array(shmax_upper,dtype='float')
    
    
    depth=depth/.3048
    shmin_lower=shmin_lower*145.038
    shmin_s=shmin_s*145.038
    shmin_upper=shmin_upper*145.038
    shmax_lower=shmax_lower*145.038
    shmax_s=shmax_s*145.038
    shmax_upper=shmax_upper*145.038
    ...
    for kk in range(0,xdim):
        for kkk in range(0,ydim):
            if strike[kk,kkk] != 0:
                ...
                abs_diff = np.abs(SoSAT_dep - Zref)
                DepIdx=np.argmin(abs_diff)
                ...
                c  = math.radians(90) #0  
                S1_= shmax_s[DepIdx]
                sH_max = shmax_upper[DepIdx]
                sH_min = shmax_lower[DepIdx]
                S2_= SvSoS[DepIdx]
                S3_= shmin_s[DepIdx]
                sh_max = shmin_upper[DepIdx]
                sh_min = shmin_lower[DepIdx]
                μ_ = 0.6
                Pp_= poreP[DepIdx] #hydrostatic
   ...
   # KrigDep=yi.reshape(xdim,ydim)
    ...
    KrigXY_return[:,0]=KrigXY_return[:,0]+np.mean(XF)
    KrigXY_return[:,1]=KrigXY_return[:,1]+np.mean(YF)
    KrigXYforMAT[ii,:,:]=KrigXY_return
    KrigZforMAT[ii,:,:]=yi
    FaultsCFF[ii,:,:,:]=coulff
...
OutputForMATLAB['KrigXY']=KrigXYforMAT
OutputForMATLAB['KrigZ']=KrigZforMAT
OutputForMATLAB['CFF']=FaultsCFF
    
    
    # ={FaultXY: test_xy, FaultZ: test_z, KrigXY: KrigXY_return, KrigZ: KrigDep, CFF: coulff}
    
sio.savemat('/Users/kmccormack/CSER Dropbox/Kevin McCormack/Utah/FaultCharacterizationProject/UpdatedHazard/Paper/MATLAB_Input_SoSAT_AllFaults.mat.mat',OutputForMATLAB)
